...
export async function POST(request: NextRequest) {
  try {
    ...
    return new NextResponse(
      JSON.stringify({ success: true, message: 'Warning email sent successfully' }),
      { status: 200, headers: { 'content-type': 'application/json' } }
    );
  } catch (error: any) {
    console.error('Send warning email error:', error);
    return new NextResponse(
      JSON.stringify({ success: false, message: 'Failed to send warning email', error: error.message }),
