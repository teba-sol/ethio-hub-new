...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!transaction) {
      console.error(`[AdminWithdrawCallback] Withdrawal transaction not found: ${txRef}`);
      return NextResponse.json({ success: false, message: 'Transaction not found' }, { status: 404 });
    }

    // If already processed, just return success
    if (transaction.status === 'COMPLETED') {
      return NextResponse.redirect(new URL('/admin/wallet?status=success', request.url));
    }
    ...
    if (data.status === 'success' && (data.data?.status === 'success' || status === 'success')) {
      ...
      console.log(`[AdminWithdrawCallback] Withdrawal successful for txRef: ${txRef}`);
      return NextResponse.redirect(new URL('/admin/wallet?status=success', request.url));
    } else {
      ...
      console.log(`[AdminWithdrawCallback] Withdrawal failed for txRef: ${txRef}. Balance restored.`);
      return NextResponse.redirect(new URL('/admin/wallet?status=failed', request.url));
    }
  } catch (error: any) {
    console.error('[AdminWithdrawCallback] Error:', error);
    return NextResponse.redirect(new URL('/admin/wallet?status=error', request.url));
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('[AdminWithdrawCallback POST] Error:', error);
